...
const navMargin = (direction, position) =>
  position === 'left'
  // && direction === "ltr"
    ? {marginLeft: `-${navbarWidth}px`} : {marginRight: `-${navbarWidth}px`}

const StyledNavBar = styled('div')(({theme, open, position}) => ({
  minWidth: navbarWidth, width: navbarWidth, maxWidth: navbarWidth, ...(!open && {
    transition: theme.transitions.create('margin', {
      easing: theme.transitions.easing.easeOut, duration: theme.transitions.duration.leavingScreen,
    }), ...navMargin(theme.direction, position),
  }), ...(open && {
    transition: theme.transitions.create('margin', {
...
const StyledNavBarMobile = styled(SwipeableDrawer)(({theme}) => ({
  '& .MuiDrawer-paper': {
    minWidth: navbarWidth, width: navbarWidth, maxWidth: navbarWidth,
  },
}));

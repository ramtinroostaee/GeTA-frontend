...
const rtlPosition = () => // direction === "rtl" ? {left: 0} :
  ({right: 0})

const rtlBorder = () => //   direction === "rtl" ? {
...
// } :
  ({
    borderTopLeftRadius: 6, borderBottomLeftRadius: 6, borderBottomRightRadius: 0, borderTopRightRadius: 0,
  });

const Root = styled('div')(({theme}) => ({
  position: 'absolute',
  height: 80, ...rtlPosition(theme.direction), ...rtlBorder(theme.direction),
  top: 160,
  display: 'flex',
  ...
  '&:hover': {
    backgroundColor: red[500], opacity: 1,
  },
...
const StyledDialog = styled(Dialog)(({theme}) => ({
  '& .MuiDialog-paper': {
    ...
    height: '100%',
    minHeight: '100%',
    bottom: 0, ...rtlPosition(theme.direction),
    margin: 0,
    zIndex: 1000,
    borderRadius: 0,
  },
